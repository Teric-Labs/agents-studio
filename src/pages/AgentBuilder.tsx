...
import { Bot, FileText, Plus, Phone, Trash2, PhoneOff, Book, Search, Bell, Menu, X, SlidersHorizontal, Copy, Workflow, ChevronLeft, ChevronRight, LogOut, RefreshCw, Save, Check, ExternalLink, Mic, Brain, Volume2, Filter, MessageSquare, MessageSquareOff, History, User, Briefcase, GraduationCap, Palette, Activity, CheckSquare, ShoppingCart, Stethoscope, Building2, Plane, Car, Settings, Globe, UtensilsCrossed, Hammer, Scale, HeartHandshake, Music, Calendar, UserCheck, Headphones, BarChart, Truck, Undo } from 'lucide-react';

import { Flex, Text, Button, Box, Grid, Card, Badge, Tabs, TextField, TextArea, Switch, Select, Slider, Heading, Separator, Tooltip, Table, Dialog, IconButton, SegmentedControl, AlertDialog } from '@radix-ui/themes';
...
export default function App() {
	...
	const toggleCall = async (targetAgentOverride?: any) => {
		...
		try {
			...
			setIsCallActive(false);
			setAgentState('disconnected');
			// showToast("Connectivity Error", "Could not reach the agent.");
		} finally {
			setIsConnecting(false);
		}
	};
		...
		<Toast.Provider swipeDirection="right">
			...
			<Box style={{
				...
				.visualizer-center-glow {
					...
					border-radius: 50%;
					z-index: 1;
					box-shadow: 0 0 40px rgba(255,255,255,0.8), 0 0 20px var(--teal-7);
			...
			`}</style>
				...
				<Box display={{ initial: 'none', lg: 'block' }} style={{ flexShrink: 0 }}>
					<Flex direction="column" style={{
						...
						<Box style={{ padding: '0 20px 24px', borderBottom: '1px solid #eceae6' }}>
							<Flex align="center" gap="3">
								<Box style={{ width: '32px', height: '32px', borderRadius: '8px', background: 'linear-gradient(135deg, var(--teal-9), var(--teal-11))', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontSize: 16 }}>E</Box>
								<Text style={{ fontWeight: 700, fontSize: 16, letterSpacing: '-0.02em', color: '#1a1a1a' }}>Etoil Studio</Text>
							</Flex>
						</Box>
							...
							{[
								...
								<Box
									...
									style={{
										...
										backgroundColor: activeView === item.id ? '#f0fdfa' : 'transparent',
										color: activeView === item.id ? 'var(--teal-9)' : '#6b7280',
						...
						<Box style={{ padding: '16px 20px', borderTop: '1px solid #eceae6', marginTop: 'auto' }}>
							<Flex align="center" gap="3" onClick={testConnection} style={{ cursor: 'pointer' }}>
								<Box style={{ width: '34px', height: '34px', borderRadius: '50%', background: 'var(--teal-9)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: 600 }}>U</Box>
								<Box>
									<Text size="2" weight="bold" as="div" style={{ lineHeight: 1 }}>User Account</Text>
									<Text size="1" style={{ color: '#64748b' }}>System Admin</Text>
								</Box>
							</Flex>
						</Box>
					</Flex>
				...
				{showMobileMenu && (
					<Box style={{
						...
						<Box style={{
							...
							<Box style={{ padding: '24px 20px 24px', borderBottom: '1px solid #eceae6' }}>
								<Flex align="center" justify="between">
									<Flex align="center" gap="3">
										<Box style={{ width: '32px', height: '32px', borderRadius: '8px', background: 'linear-gradient(135deg, var(--teal-9), var(--teal-11))', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700 }}>E</Box>
										<Text style={{ fontWeight: 700 }}>Etoil Studio</Text>
									</Flex>
							...
							<Box style={{ padding: '16px 20px', borderTop: '1px solid #eceae6' }}>
								<Flex align="center" gap="3">
									<Box style={{ width: '34px', height: '34px', borderRadius: '50%', background: 'var(--teal-9)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>U</Box>
									<Box>
										<Text size="2" weight="bold" as="div" style={{ color: '#1e293b' }}>User Account</Text>
										<Text size="1" style={{ color: '#64748b' }}>System Admin</Text>
									</Box>
								</Flex>
							</Box>
								...
								{activeView === 'workflows' && editingWorkflowId ? (
									<Flex align="center" gap="3">
										...
										<TextField.Root
											...
											size="2"
											variant="soft"
											style={{ fontWeight: 800, fontSize: '18px', color: '#1e293b', minWidth: '300px' }}
										/>
									</Flex>
								) : (
									<>
										<Heading size="5" style={{ margin: 0, fontWeight: 800, color: '#1e293b', letterSpacing: '-0.02em' }}>
										...
										<Box display={{ initial: 'none', sm: 'block' }}>
											<Text size="1" style={{ color: '#64748b', marginTop: '2px', fontWeight: 500 }}>{new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })} — Welcome back.</Text>
										</Box>
									</>
								)}
							</Box>
						...
						<Flex align="center" gap="4">
							<Box display={{ initial: 'none', md: 'block' }}>
								<Flex align="center" gap="2">
									<Text size="1" style={{ color: '#64748b' }}>Status</Text>
									<Badge color={connectionStatus === 'connected' ? 'teal' : 'gray'} variant="soft">
							...
							{activeView === 'workflows' ? (
								<Flex gap="3" align="center" wrap="wrap" justify={{ initial: 'start', sm: 'end' }}>
									{editingWorkflowId && <Button variant="ghost" style={{ color: '#4b5563' }} onClick={() => setEditingWorkflowId(null)} size="2"><LogOut size={16} /> Exit Designer</Button>}
									<Button variant="ghost" style={{ color: '#4b5563' }} onClick={() => loadWorkflowsList()} size="2"><RefreshCw size={16} /> Reload</Button>
									{editingWorkflowId && (
								...
								<Flex align="center" gap="4">
									<Search size={18} color="#6b7280" style={{ cursor: 'pointer' }} />
							...
							<Flex direction="column" gap="6">
								...
								<Grid columns={{ initial: '1', sm: '2', lg: '4' }} gap="4">
									{[
										...
										<Card key={i} size="2" style={{ borderRadius: 'var(--radius-2)', border: '1px solid #e8e5e0', padding: '16px' }}>
											<Flex direction="column" gap="1">
												<Text size="2" style={{ color: '#0f172a', fontWeight: 800 }}>{metric.label}</Text>
												<Heading size="6" weight="bold" style={{ color: '#1e293b' }}>{metric.value}</Heading>
												<Flex gap="1" align="center" mt="1">
													<Badge color={metric.up ? 'teal' : 'amber'} variant="soft" radius="full">
														{metric.change}
													</Badge>
												</Flex>
											</Flex>
								...
								<Grid columns={{ initial: '1', lg: '1fr 340px' }} gap="4">
									<Box style={{ backgroundColor: 'white', border: '1px solid #e8e5e0', borderRadius: '14px', padding: '24px' }}>
										<Heading size="4" mb="1" style={{ color: '#0f172a', fontWeight: 800 }}>Performance Overview</Heading>
										<Text size="1" style={{ color: '#64748b', marginBottom: '20px', display: 'block', fontWeight: 500 }}>Last 7 days success metrics</Text>
										<Flex align="end" gap="2" style={{ height: '120px' }}>
											{[55, 72, 61, 88, 76, 95, 82].map((h, index) => (
												<Box key={index} style={{ flexGrow: 1, backgroundColor: index === 5 ? 'var(--teal-9)' : '#eeece8', height: `${h}%`, borderRadius: '4px 4px 0 0', transition: 'height 0.3s' }} />
											))}
										</Flex>
									</Box>

									<Box style={{ backgroundColor: 'white', border: '1px solid #e8e5e0', borderRadius: '14px', padding: '24px' }}>
										<Heading size="4" mb="4" style={{ color: '#0f172a', fontWeight: 700 }}>System Usage</Heading>
										<Flex direction="column" gap="4">
											{[
												{ label: 'API Bandwidth', value: 62, color: 'var(--teal-9)' },
												{ label: 'Token Utilization', value: 48, color: '#f59e0b' },
												{ label: 'Concurrency', value: 35, color: '#10b981' }
											].map(item => (
												<Box key={item.label}>
													<Flex justify="between" mb="1" style={{ fontSize: '11px', fontWeight: 'bold', textTransform: 'uppercase' }}>
														...
														<Text style={{ color: '#1e293b' }}>{item.value}%</Text>
													</Flex>
													<Box style={{ height: '6px', borderRadius: '3px', backgroundColor: '#e5e7eb', overflow: 'hidden' }}>
								...
								{!editingWorkflowId ? (
									<Card size="2" style={{ borderRadius: 'var(--radius-2)', backgroundColor: 'white', border: '1px solid #e8e5e0' }}>
										<Flex direction={{ initial: 'column', md: 'row' }} justify="between" align={{ initial: 'stretch', md: 'center' }} gap="4" mb="5">
											<Box>
												<Heading size={{ initial: '3', md: '4' }} mb="1" style={{ color: '#0f172a', fontWeight: 800 }}>Workflows</Heading>
												<Text size={{ initial: '1', md: '2' }} style={{ color: '#64748b', fontWeight: 500 }}>Design complex logic for your AI agents</Text>
											</Box>
											<Flex gap="3" direction={{ initial: 'column', md: 'row' }} align={{ initial: 'stretch', md: 'center' }}>
												...
												<Button color="teal" size="2" onClick={() => {
													setWorkflowsPayload(null);
											...
											<Table.Root variant="ghost" size="1">
												...
												<Table.Body>
														...
														<Table.Row key={wf.id} align="center">
															<Table.Cell>
																<Flex align="center" gap="3">
																	...
																	<Box>
																		<Text size="2" weight="bold" style={{ display: 'block', color: '#1e293b' }}>{wf.name}</Text>
																		<Text size="1" weight="medium" style={{ color: '#475569', display: 'block', maxWidth: '300px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{wf.description || 'No description'}</Text>
																	</Box>
																</Flex>
															</Table.Cell>
															<Table.Cell>
																<Tooltip content={wf.id}>
																	<Text size="1" style={{ color: '#475569', fontFamily: 'monospace', fontWeight: 500, backgroundColor: '#f8fafc', padding: '2px 6px', borderRadius: '4px', cursor: 'help' }}>
																		{wf.id.substring(0, 8)}...
																	</Text>
																</Tooltip>
															</Table.Cell>
															...
															<Table.Cell><Text size="2" style={{ color: '#475569' }}>{new Date(wf.created_at).toLocaleDateString()}</Text></Table.Cell>
															<Table.Cell>
																<Flex gap="2" justify="center" align="center">
																	...
																	<AlertDialog.Root>
																		...
																		<AlertDialog.Content maxWidth="450px">
																			<AlertDialog.Title style={{ color: '#1e293b', fontWeight: 800 }}>Delete Workflow</AlertDialog.Title>
																			...
																			<Flex gap="3" mt="4" justify="end" align="center">
																				<AlertDialog.Cancel>
																					<Button variant="soft" color="gray"><X size={16} /> Cancel</Button>
																				</AlertDialog.Cancel>
																				<AlertDialog.Action>
													...
													{workflowsList.length === 0 && (
														<Table.Row><Table.Cell colSpan={5} style={{ textAlign: 'center', padding: '40px', color: '#475569' }}>No workflows found. Design your first logic graph.</Table.Cell></Table.Row>
													)}
												</Table.Body>
											</Table.Root>
										</Box>
										...
										{totalWorkflowPages > 1 && (
											<Flex justify="between" align="center" mt="5" pt="4" style={{ borderTop: '1px solid #f1f5f9' }}>
												<Text size="1" color="gray">Showing {(workflowPage - 1) * WORKFLOW_PAGE_SIZE + 1} to {Math.min(workflowPage * WORKFLOW_PAGE_SIZE, filteredWorkflows.length)} of {filteredWorkflows.length}</Text>
												<Flex gap="2">
													<IconButton size="1" variant="soft" color="gray" disabled={workflowPage === 1} onClick={() => setWorkflowPage(p => p - 1)}><ChevronLeft size={14} /></IconButton>
													<Text size="2" weight="bold" style={{ padding: '0 8px' }}>{workflowPage} / {totalWorkflowPages}</Text>
													<IconButton size="1" variant="soft" color="gray" disabled={workflowPage === totalWorkflowPages} onClick={() => setWorkflowPage(p => p + 1)}><ChevronRight size={14} /></IconButton>
												</Flex>
											</Flex>
										)}
									</Card>
				...
				<Card size="2" style={{ borderRadius: 'var(--radius-2)', backgroundColor: 'white', border: '1px solid #e8e5e0', padding: '24px' }}>
					...
					<Flex direction={{ initial: 'column', md: 'row' }} justify="between" align={{ initial: 'stretch', md: 'center' }} gap="4" mb="5">
						<Box>
							<Heading size={{ initial: '3', md: '4' }} mb="1" style={{ color: '#1e293b', fontWeight: 800 }}>Conversation History</Heading>
							<Text size="2" style={{ color: '#64748b', fontWeight: 500 }}>{filteredLogs.length} session{filteredLogs.length !== 1 ? 's' : ''} recorded</Text>
						</Box>
						<Flex gap="3" wrap="wrap" align="center">
							...
							{/* Refresh */}
							<Button variant="soft" color="gray" size="2" onClick={loadLogs} loading={isLoadingLogs}>
								...
								{isLoadingLogs ? (
									<Table.Row>
										<Table.Cell colSpan={6} style={{ textAlign: 'center', padding: '48px' }}>
											<Flex justify="center" align="center" gap="2"><RefreshCw size={18} className="animate-spin" color="#94a3b8" /><Text size="2" color="gray">Loading history...</Text></Flex>
										</Table.Cell>
									</Table.Row>
									...
									<Table.Row>
										<Table.Cell colSpan={6} style={{ textAlign: 'center', padding: '64px' }}>
											<Flex direction="column" align="center" gap="2">
												...
												<Text size="2" weight="bold" style={{ color: '#475569' }}>No history found</Text>
												<Text size="1" color="gray">Sessions will appear here after interactions complete.</Text>
											</Flex>
										</Table.Cell>
									</Table.Row>
									...
									<Table.Row key={log.id} align="center">
										...
										<Table.Cell>
											<Tooltip content={log.session_id}>
												<Text size="1" style={{ color: '#475569', fontFamily: 'monospace', backgroundColor: '#f8fafc', padding: '2px 6px', borderRadius: '4px', cursor: 'help' }}>
										...
										<Table.Cell>
											<Flex align="center" gap="2">
												...
												<Text size="2" weight="medium" style={{ color: '#1e293b' }}>{log.agent_name}</Text>
											</Flex>
										</Table.Cell>
										{/* Type */}
										<Table.Cell>
											<Flex align="center" gap="2">
												...
												<Badge color={log.conversation_type === 'chat' ? 'blue' : 'teal'} variant="soft" radius="full">
										...
										<Table.Cell>
											<Flex direction="column" gap="1">
												<Text size="2" style={{ color: '#0f172a', fontWeight: 600 }}>{log.message_count} turns</Text>
												<Text size="1" color="gray">{log.user_turn_count}u / {log.agent_turn_count}a</Text>
											</Flex>
										</Table.Cell>
										{/* Time */}
										<Table.Cell>
											<Flex direction="column" gap="1">
												<Text size="2" style={{ color: '#1e293b' }}>{log.created_at ? new Date(log.created_at).toLocaleDateString() : '—'}</Text>
												<Text size="1" color="gray">{log.created_at ? new Date(log.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''}</Text>
											</Flex>
										</Table.Cell>
										{/* Status */}
										<Table.Cell>
											<Badge color="teal" variant="soft" radius="full" style={{ fontWeight: 700 }}>
												● COMPLETED
											</Badge>
										</Table.Cell>
										<Table.Cell>
					...
					{totalLogPages > 1 && (
						<Flex justify="between" align="center" mt="5" pt="4" style={{ borderTop: '1px solid #f1f5f9' }}>
							<Text size="1" color="gray">Showing {(logPage - 1) * LOG_PAGE_SIZE + 1}–{Math.min(logPage * LOG_PAGE_SIZE, filteredLogs.length)} of {filteredLogs.length} sessions</Text>
							<Flex gap="2">
								<IconButton size="1" variant="soft" color="gray" disabled={logPage === 1} onClick={() => setLogPage(p => p - 1)}><ChevronLeft size={14} /></IconButton>
								<Text size="2" weight="bold" style={{ padding: '0 8px', color: '#1e293b' }}>{logPage} / {totalLogPages}</Text>
								<IconButton size="1" variant="soft" color="gray" disabled={logPage === totalLogPages} onClick={() => setLogPage(p => p + 1)}><ChevronRight size={14} /></IconButton>
							</Flex>
						</Flex>
					)}
				</Card>
							...
							<Box>
								<Card size="2" style={{ borderRadius: 'var(--radius-2)', backgroundColor: 'white', border: '1px solid #e8e5e0', padding: '24px' }}>
									<Flex direction={{ initial: 'column', md: 'row' }} justify="between" align={{ initial: 'stretch', md: 'center' }} gap="4" mb="5">
										<Box>
											<Heading size={{ initial: '3', md: '4' }} mb="1" style={{ color: '#1e293b', fontWeight: 800 }}>AI Agents</Heading>
											<Text size={{ initial: '1', md: '2' }} style={{ color: '#64748b', fontWeight: 500 }}>Manage your fleet of deployed voice assistants</Text>
										</Box>
										<Flex gap="3" direction={{ initial: 'column', md: 'row' }} align={{ initial: 'stretch', md: 'center' }}>
											...
											<Button color="teal" size="2" variant="solid" onClick={openNewAgentBuilder} style={{ borderRadius: 'var(--radius-1)', fontWeight: 600 }}>
										...
										<Table.Root variant="ghost" size="1">
											...
											<Table.Body>
												{paginatedAgents.map((agent) => (
													<Table.Row key={agent.id} align="center" style={{ cursor: 'pointer', backgroundColor: currentAgent?.id === agent.id ? '#f0fdf9' : 'transparent', borderLeft: currentAgent?.id === agent.id ? '4px solid var(--teal-9)' : 'none' }} onClick={() => loadAgent(agent.id, false)}>
														<Table.Cell>
															<Flex align="center" gap="3">
																<Box style={{ width: '36px', height: '36px', borderRadius: 'var(--radius-1)', backgroundColor: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
																	<Bot size={20} style={{ color: '#475569' }} />
																</Box>
																<Box>
														...
														<Table.Cell>
															<Flex align="center" gap="2">
																<Tooltip content={agent.id}>
																	<Text size="1" style={{ color: '#475569', fontFamily: 'monospace', fontWeight: 500, backgroundColor: '#f8fafc', padding: '2px 6px', borderRadius: '4px' }}>
														...
														<Table.Cell>
															<Badge color={agent.config?.category === 'business' ? 'orange' : agent.config?.category === 'personal' ? 'teal' : 'gray'} variant="soft">
																{(agent.config?.category || 'blank').toUpperCase()}
															</Badge>
														</Table.Cell>
														<Table.Cell>
															<Text size="1" style={{ color: '#64748b', fontWeight: 500 }}>
														...
														<Table.Cell>
															<Flex gap="2" justify="center" align="center">
																<Button
																	...
																	color={isCallActive && currentAgent?.id === agent.id ? "red" : "teal"}
																	size="1"
																...
																<Button variant="ghost" color="gray" size="1" onClick={(e) => { e.stopPropagation(); loadAgent(agent.id, true); }}><SlidersHorizontal size={14} /> Config</Button>
																...
																<AlertDialog.Root>
																	...
																	<AlertDialog.Content maxWidth="450px" onClick={(e) => e.stopPropagation()}>
																		<AlertDialog.Title style={{ color: '#1e293b', fontWeight: 800 }}>Delete Agent</AlertDialog.Title>
																		<AlertDialog.Description size="2">
												...
												{agentsList.length === 0 && (
													<Table.Row>
														<Table.Cell colSpan={5} style={{ textAlign: 'center', padding: '40px' }}>
															<Text size="2" style={{ color: '#475569' }}>No agents found. Create one to start testing.</Text>
														</Table.Cell>
													</Table.Row>
												)}
											</Table.Body>
									...
									{totalAgentPages > 1 && (
										<Flex justify="between" align="center" mt="5" pt="4" style={{ borderTop: '1px solid #f1f5f9' }}>
											<Text size="1" color="gray">Agents {(agentPage - 1) * AGENT_PAGE_SIZE + 1}-{Math.min(agentPage * AGENT_PAGE_SIZE, filteredAgents.length)} of {filteredAgents.length}</Text>
											<Flex gap="2">
												<IconButton size="1" variant="soft" color="gray" disabled={agentPage === 1} onClick={() => setAgentPage(p => p - 1)}><ChevronLeft size={14} /></IconButton>
												<Text size="2" weight="bold" style={{ padding: '0 8px', color: '#1e293b' }}>{agentPage}</Text>
												<IconButton size="1" variant="soft" color="gray" disabled={agentPage === totalAgentPages} onClick={() => setAgentPage(p => p + 1)}><ChevronRight size={14} /></IconButton>
											</Flex>
										</Flex>
									)}
								</Card>
					...
					{/* Live Voice/Chat Interaction Dialog */}
					<Dialog.Root open={isVoiceUIOpen} onOpenChange={setIsVoiceUIOpen}>
						<Dialog.Content size="3" style={{ maxWidth: '440px', padding: 0, borderRadius: '16px', overflow: 'hidden' }}>
							<div style={{ height: '600px', display: 'flex', flexDirection: 'column', backgroundColor: '#ffffff' }}>
								{currentAgent ? (
									<>
										{/* Header */}
										<div style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '14px 20px', borderBottom: '1px solid #e2e8f0', flexShrink: 0, backgroundColor: '#f8fafc' }}>
											<div style={{ position: 'relative', width: '44px', height: '44px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
												{isCallActive && <div className="visualizer-center-glow" style={{ width: '100%', height: '100%', filter: 'blur(8px)', position: 'absolute' }} />}
												{!isCallActive && <div style={{ width: '40px', height: '40px', borderRadius: '50%', border: '2px dashed #cbd5e1', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Phone size={18} color="#94a3b8" /></div>}
												{isCallActive && <div style={{ position: 'absolute', transform: 'scale(0.24)', transformOrigin: 'center', zIndex: 2 }}><AgentAudioVisualizerBar audioTrack={agentAudioTrack} state={agentState} /></div>}
											</div>
											<div style={{ flex: 1 }}>
												<Text size="3" weight="bold" style={{ color: '#0f172a', textTransform: 'uppercase', letterSpacing: '0.05em', display: 'block' }}>{currentAgent?.name}</Text>
												<Badge color={isCallActive ? "teal" : isChatActive ? "blue" : "gray"} variant="soft" radius="full" size="1" style={{ fontWeight: 800, marginTop: '4px' }}>{isCallActive ? 'VOICE ACTIVE' : isChatActive ? 'CHAT ACTIVE' : 'READY'}</Badge>
											</div>
											<Dialog.Close>
												<IconButton variant="ghost" color="gray"><X size={18} /></IconButton>
											</Dialog.Close>
										</div>
										{/* Chat body */}
										<div style={{ flex: 1, backgroundColor: '#f8fafc', overflowY: 'auto' }}>
											{transcripts.length > 0 ? (
												<AgentChatTranscript messages={transcripts} agentState={agentState} />
											) : (
												<div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', opacity: 0.55 }}>
													...
													<Text size="2" weight="bold" color="gray">No Messages Yet</Text>
													{!isCallActive && !isChatActive && <Text size="1" mt="1" color="gray">Start a voice call or text chat below.</Text>}
												</div>
											)}
										</div>
										{/* Bottom action bar */}
										<div style={{ borderTop: '1px solid #e2e8f0', backgroundColor: '#ffffff', flexShrink: 0 }}>
											{(isCallActive || isChatActive) && (
												<form onSubmit={(e) => { e.preventDefault(); sendChatMessage(); }} style={{ display: 'flex', gap: '8px', padding: '10px 16px', alignItems: 'center' }}>
													<input
														type="text"
														value={chatInput}
														onChange={e => setChatInput(e.target.value)}
														placeholder="Type a message..."
														style={{ flex: 1, padding: '10px 16px', borderRadius: '24px', border: '1.5px solid #e2e8f0', outline: 'none', fontSize: '13px', backgroundColor: '#f8fafc', transition: 'border-color 0.2s, box-shadow 0.2s', fontFamily: 'inherit' }}
														onFocus={(e) => { e.currentTarget.style.borderColor = 'var(--teal-7)'; e.currentTarget.style.boxShadow = '0 0 0 3px var(--teal-3)'; }}
														onBlur={(e) => { e.currentTarget.style.borderColor = '#e2e8f0'; e.currentTarget.style.boxShadow = 'none'; }}
													/>
													<button
														type="submit"
														disabled={!chatInput.trim()}
														style={{ width: '36px', height: '36px', borderRadius: '50%', border: 'none', backgroundColor: chatInput.trim() ? 'var(--teal-9)' : '#e2e8f0', color: chatInput.trim() ? 'white' : '#94a3b8', cursor: chatInput.trim() ? 'pointer' : 'default', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s', flexShrink: 0 }}
													>
														<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>
													</button>
												</form>
											)}
											<div style={{ display: 'flex', gap: '8px', padding: '10px 16px', justifyContent: 'center' }}>
												<button
													onClick={() => toggleCall()}
													...
													style={{ 
														...
														backgroundColor: isCallActive ? '#fee2e2' : '#f0fdfa', 
														color: isCallActive ? '#dc2626' : 'var(--teal-9)', 
														fontWeight: 600, 
														fontSize: '13px', 
														transition: 'all 0.2s', 
														opacity: (isChatActive || currentAgent?.config?.chat_only) ? 0.5 : 1, 
														fontFamily: 'inherit' 
													}}
												>
													{isConnecting ? <RefreshCw size={14} className="animate-spin" /> : isCallActive ? <PhoneOff size={14} /> : <Phone size={14} />}
													{isConnecting ? 'Connecting...' : isCallActive ? 'End Call' : (currentAgent?.config?.chat_only ? 'Chat Only' : 'Voice Call')}
												</button>
												<button
													onClick={() => toggleChatSession()}
													disabled={isChatConnecting || isConnecting || isCallActive}
													style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', padding: '10px', borderRadius: '10px', border: 'none', cursor: (isChatConnecting || isCallActive) ? 'not-allowed' : 'pointer', backgroundColor: isChatActive ? '#fee2e2' : '#eff6ff', color: isChatActive ? '#dc2626' : '#2563eb', fontWeight: 600, fontSize: '13px', transition: 'all 0.2s', opacity: isCallActive ? 0.5 : 1, fontFamily: 'inherit' }}
												>
													{isChatConnecting ? <RefreshCw size={14} className="animate-spin" /> : isChatActive ? <X size={14} /> : <MessageSquare size={14} />}
									...
									<div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
										<div style={{ width: '64px', height: '64px', borderRadius: '50%', backgroundColor: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#94a3b8', marginBottom: '14px' }}><Bot size={32} /></div>
										<Text size="2" weight="bold" style={{ color: '#1e293b' }}>NO AGENT SELECTED</Text>
										<Text size="1" style={{ color: '#94a3b8', marginTop: '4px' }}>Click an agent's row to start monitoring.</Text>
									</div>
								)}
							</div>
						</Dialog.Content>
					...
					<Dialog.Root open={isBuilderOpen} onOpenChange={setIsBuilderOpen}>
						<Dialog.Content size="4" style={{ maxWidth: '1200px', padding: 0 }}>
							<Flex direction="column" style={{ height: '85vh', overflow: 'hidden' }}>
								<Box p="4" style={{ borderBottom: '1px solid #f0ede8', backgroundColor: '#f9f8f6' }}>
									<Flex justify="between" align="center">
										<Box>
											<Flex align="center" gap="3" mb="1">
												<Box style={{ backgroundColor: 'var(--teal-3)', color: 'var(--teal-9)', padding: '6px', borderRadius: '8px' }}>
													<Bot size={24} />
												</Box>
											...
											<Dialog.Description size="2" style={{ color: '#64748b', marginLeft: '45px' }}>
										...
										<Flex gap="3" align="center">
											<Dialog.Close>
												<Button variant="ghost" size="2" style={{ color: '#4b5563' }}><X size={16} /> Cancel</Button>
											</Dialog.Close>
											{creationStep === 'CONFIG' && (
												<Button color="teal" size="2" onClick={() => createAgent()} loading={isLoading}>
								...
								<Box style={{ flexGrow: 1, overflowY: 'auto', backgroundColor: '#fcfbf9' }}>
									<Box p="6">
										{creationStep === 'CATEGORY' && (
											<Flex direction="column" align="center" gap="6" py="8">
												<Box style={{ textAlign: 'center' }}>
													<Heading size="8" style={{ fontWeight: 800, color: '#0f172a' }}>New agent</Heading>
													<Text size="3" style={{ color: '#64748b' }}>What type of agent would you like to create?</Text>
												</Box>

												<Flex direction="column" gap="4" style={{ width: '100%', maxWidth: '800px' }}>
													<Card size="3" onClick={() => { setAgentCategory('blank'); setCreationStep('CONFIG'); }} style={{ cursor: 'pointer', transition: 'all 0.2s', border: '1px solid #e2e8f0' }} className="category-card">
														<Flex align="center" justify="center" gap="3">
															<Box style={{ color: '#94a3b8' }}><History size={24} style={{ border: '2px dashed currentColor', borderRadius: '50%', padding: '4px' }} /></Box>
															<Text size="4" weight="bold">Blank Agent</Text>
														</Flex>
													</Card>

													<Grid columns="2" gap="4">
														<Card size="3" onClick={() => { setAgentCategory('personal'); setCreationStep('PERSONAL_USE_CASE'); }} style={{ cursor: 'pointer', transition: 'all 0.2s', border: '1px solid #e2e8f0', minHeight: '300px' }} className="category-card">
															<Flex direction="column" gap="4" align="center" justify="center" style={{ height: '100%' }}>
																<Box style={{ width: '100%', maxWidth: '280px', height: '180px', backgroundColor: '#f8fafc', borderRadius: '12px', padding: '12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
																	<Flex justify="end"><Box style={{ fontSize: '11px', backgroundColor: '#0f172a', color: 'white', padding: '4px 10px', borderRadius: '12px', fontWeight: 600 }}>Could you see whether I have any urgent outstanding emails?</Box></Flex>
																	<Box style={{ fontSize: '11px', backgroundColor: '#f1f5f9', color: '#64748b', padding: '4px 10px', borderRadius: '12px', alignSelf: 'start', maxWidth: '80%' }}>Sure, let me check.</Box>
																	<Box style={{ fontSize: '11px', backgroundColor: '#f1f5f9', color: '#64748b', padding: '4px 10px', borderRadius: '12px', alignSelf: 'start', maxWidth: '90%' }}>You've got one urgent email from your manager about tomorrow's meeting. Want a quick summary?</Box>
																</Box>
																<Flex align="center" gap="2">
																	<User size={18} color="#64748b" />
																	<Text size="3" weight="bold">Personal Assistant</Text>
																</Flex>
															</Flex>
														</Card>

														<Card size="3" onClick={() => { setAgentCategory('business'); setCreationStep('BUSINESS_INDUSTRY'); }} style={{ cursor: 'pointer', transition: 'all 0.2s', border: '1px solid #e2e8f0', minHeight: '300px' }} className="category-card">
															<Flex direction="column" gap="4" align="center" justify="center" style={{ height: '100%' }}>
																<Box style={{ width: '100%', maxWidth: '280px', height: '180px', backgroundColor: '#f8fafc', borderRadius: '12px', padding: '12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
																	<Flex justify="end"><Box style={{ fontSize: '11px', backgroundColor: '#0f172a', color: 'white', padding: '4px 10px', borderRadius: '12px', fontWeight: 600 }}>Can you tell me more about pricing?</Box></Flex>
																	<Box style={{ fontSize: '11px', backgroundColor: '#f1f5f9', color: '#64748b', padding: '4px 10px', borderRadius: '12px', alignSelf: 'start', maxWidth: '85%' }}>Absolutely! We offer three plans, Starter, Pro, and Enterprise. Want a quick breakdown, or should I help you pick the best fit?</Box>
																</Box>
																<Flex align="center" gap="2">
																	<Briefcase size={18} color="#64748b" />
																	<Text size="3" weight="bold">Business Agent</Text>
																	<Badge color="green" variant="soft" radius="full">Improved</Badge>
																</Flex>
															</Flex>
														</Card>
													</Grid>
										...
										{creationStep === 'PERSONAL_USE_CASE' && (
											<Flex direction="column" gap="6">
												<Box>
													<Heading size="7" style={{ fontWeight: 800, color: '#0f172a' }}>Use case</Heading>
													<Text size="3" style={{ color: '#64748b' }}>What will your agent help with?</Text>
												</Box>
												<Grid columns="3" gap="4">
													{[
														...
														<Card 
															...
															style={{ cursor: 'pointer', border: item.dashed ? '1px dashed #cbd5e1' : '1px solid #e2e8f0', transition: 'all 0.2s' }}
															className="category-card"
														>
															<Flex direction="column" align="center" justify="center" gap="3" py="4">
																<Box style={{ color: '#64748b' }}>{item.icon}</Box>
												...
												<Box mt="4">
													<Button variant="outline" color="gray" onClick={() => setCreationStep('CATEGORY')} size="2">
										...
										{creationStep === 'BUSINESS_INDUSTRY' && (
											<Flex direction="column" gap="6">
												<Box>
													<Heading size="7" style={{ fontWeight: 800, color: '#0f172a' }}>What industry is your business in?</Heading>
													<Text size="3" style={{ color: '#64748b' }}>Select the industry that best describes your business</Text>
												</Box>
												<Grid columns="3" gap="4">
													{[
										...
										{creationStep === 'BUSINESS_USE_CASE' && (
											<Flex direction="column" gap="6">
												...
												<Box mt="4">
													<Button variant="outline" color="gray" onClick={() => setCreationStep('BUSINESS_INDUSTRY')} size="2">
														<ChevronLeft size={16} /> Back
													</Button>
												</Box>
											</Flex>
											...
											<Tabs.Root defaultValue="instructions">
														...
														<Flex direction="column" gap="4">
															...
															<Box style={{ backgroundColor: '#f8fafc', padding: '12px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
																<Flex align="center" gap="3">
																	...
																	<Box>
																		<Text size="2" weight="bold" style={{ color: '#0f172a' }}>Chat only</Text>
																		<Text size="1" color="gray" ml="2">Audio will not be processed and only text will be used</Text>
																	</Box>
																</Flex>
															</Box>
													...
													<Grid columns="3" gap="5">
														<Flex direction="column" gap="4">
															<Flex align="center" gap="2" mb="1">
																<Box style={{ backgroundColor: '#f0f9ff', padding: '6px', borderRadius: '6px', color: '#0369a1' }}>
																	<Mic size={18} />
																</Box>
																<Heading size="3" style={{ color: '#0369a1', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.02em' }}>Transcription (STT)</Heading>
															</Flex>
														...
														<Flex direction="column" gap="4" style={{ padding: '0 10px', borderLeft: '1px solid #f0ede8', borderRight: '1px solid #f0ede8' }}>
															<Flex align="center" gap="2" mb="1">
																<Box style={{ backgroundColor: '#fdf4ff', padding: '6px', borderRadius: '6px', color: '#a21caf' }}>
																	<Brain size={18} />
																</Box>
																<Heading size="3" style={{ color: '#a21caf', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.02em' }}>Reasoning (LLM)</Heading>
															</Flex>
															...
															<Flex direction="column" gap="2">
																<Text size="2" weight="bold">Temperature: {providerConfigs.llm.temperature || 0.7}</Text>
																<Slider defaultValue={[0.7]} max={1} step={0.1} onValueChange={([v]) => updateProviderConfig('llm', 'temperature', v.toString())} />
															</Flex>
														</Flex>

														<Flex direction="column" gap="4">
															<Flex align="center" gap="2" mb="1">
																<Box style={{ backgroundColor: '#fff7ed', padding: '6px', borderRadius: '6px', color: '#c2410c' }}>
																	<Volume2 size={18} />
																</Box>
																<Heading size="3" style={{ color: '#c2410c', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.02em' }}>Speech (TTS)</Heading>
															</Flex>
												...
												<Tabs.Content value="logic">
													<Flex direction="column" gap="5" p="2">
														<Box>
															<Text as="label" size="2" weight="bold" mb="3" style={{ display: 'block', color: '#1e293b' }}>
														...
														{agentType === 'workflow' ? (
															<Card size="1" style={{ backgroundColor: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '12px' }}>
																<Flex direction="column" gap="3" p="3">
																	<Box>
																		<Text size="2" weight="bold" mb="2" style={{ display: 'block', color: '#1e293b' }}>Attach Saved Workflow</Text>
															...
															<Box style={{ padding: '24px', border: '2px dashed #f1f5f9', borderRadius: '12px', textAlign: 'center' }}>
																<Text size="2" color="gray">In <b>General Mode</b>, logic is driven by the prompt in the <b>Core Behavior</b> tab.</Text>
															</Box>
														)}
													</Flex>
												</Tabs.Content>
					...
					<Dialog.Root open={isTranscriptModalOpen} onOpenChange={setIsTranscriptModalOpen}>
						<Dialog.Content size="3" style={{ maxWidth: '600px', padding: 0, borderRadius: '16px', overflow: 'hidden' }}>
							<Box p="4" style={{ borderBottom: '1px solid #f1f5f9', backgroundColor: '#f8fafc' }}>
								<Flex justify="between" align="center">
									<Box>
										<Flex align="center" gap="2" mb="1">
											<div style={{ backgroundColor: 'var(--indigo-3)', color: 'var(--indigo-9)', padding: '4px', borderRadius: '6px' }}>
												<MessageSquare size={18} />
											</div>
											<Dialog.Title style={{ margin: 0, fontSize: '18px', fontWeight: 800, color: '#1e293b' }}>Conversation Detail</Dialog.Title>
										</Flex>
							...
							<Box style={{ height: '500px', overflowY: 'auto', backgroundColor: '#ffffff' }}>
								{selectedLogForTranscript?.transcripts && selectedLogForTranscript.transcripts.length > 0 ? (
									<div style={{ padding: '20px' }}>
										<AgentChatTranscript messages={selectedLogForTranscript.transcripts} agentState="disconnected" />
									</div>
								) : (
									<Flex direction="column" align="center" justify="center" style={{ height: '100%', opacity: 0.5 }}>
										<MessageSquareOff size={48} color="#cbd5e1" style={{ marginBottom: '16px' }} />
										<Text size="2" weight="bold" color="gray">No Transcripts Available</Text>
										<Text size="1" color="gray">This session data is missing conversation detail.</Text>
									</Flex>
								)}
							</Box>

							<Box p="3" style={{ borderTop: '1px solid #f1f5f9', backgroundColor: '#f8fafc' }}>
								<Flex justify="end">
									<Dialog.Close>
										<Button variant="solid" color="gray" size="2">Close Overview</Button>
									</Dialog.Close>
								</Flex>
							</Box>
						</Dialog.Content>
